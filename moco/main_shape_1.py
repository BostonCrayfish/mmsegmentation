...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.head == 'fcn':
        ...
        args.mask_file = config_dir + 'masks.pth'
...
def train(train_loader_list, model, criterion, optimizer, epoch, args):
    ...
    mask_total = torch.load(args.mask_file)
    for i, ((images, _), (bg0, _), (bg1, _)) in enumerate(zip(train_loader, train_loader_bg0, train_loader_bg1)):
        ...
        image_q = images[0] * mask_q.unsqueeze(1) + bg0 * (1. - mask_q.unsqueeze(1))
        image_k = images[1] * mask_k.unsqueeze(1) + bg1 * (1. - mask_k.unsqueeze(1))
