...
def train(train_loader_list, model, criterion, optimizer, epoch, args):
    ...
    for i, ((images, _), (bg0, _), (bg1, _)) in enumerate(zip(train_loader, train_loader_bg0, train_loader_bg1)):
        ...
        mask_q = msk_gen_q(torch.zeros(1, 224, 224))[0]
        mask_k = msk_gen_k(torch.zeros(1, 224, 224))[0]
        ...
        image_q = images[0] * mask_q + bg0 * (1 - mask_q)
        ...
        acc_dense_pos = output_dense.argmax(dim=1).reshape(-1)
        # print(acc_dense_pos[torch.randint(high=len(acc_dense_pos), size=(1, 20)).to('cuda')])
        print(target_dense.shape, target_dense.sum())
