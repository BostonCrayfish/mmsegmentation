...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = datasets.ImageFolder(traindir, loader=loader_non_random)
...
def train(train_loader_list, model, criterion, optimizer, epoch, args):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, loss_m, loss_s, acc_moco],
    ...
    for i, ((images, _), (bg0, _), (bg1, _)) in enumerate(zip(train_loader, train_loader_bg0, train_loader_bg1)):
        ...
        mask_q[images[0][:, 0, :, :] == 0.] = 0.
        mask_k[images[1][:, 0, :, :] == 0.] = 0.
        ...
        image_q = images[0] + bg0 * (1. - mask_q.unsqueeze(1))
        image_k = images[1] + bg1 * (1. - mask_k.unsqueeze(1))
