...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, mask_q, mask_k):
        ...
        # idx_qpos = torch.where(mask_q == 1)
        # idx_kpos = torch.where(mask_k == 1)
        ...
        # labels_dense = mask_q
        # mask_dense = torch.tensor(1.).cuda()    # no logit masked
        ...
        print('grad:', self.encoder_q.backbone.blocks[0].norm1.weight.grad)
        print('grad_fn:', self.encoder_q.backbone.blocks[0].norm1.weight.grad_fn)
