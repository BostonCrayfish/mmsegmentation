...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train([train_loader, train_loader_bg0, train_loader_bg1], model, optimizer, epoch, args)
        if epoch % 5 == 4:
...
def train(train_loader_list, model, optimizer, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    loss_b = AverageMeter('Loss_byol', ':.4e')
    loss_s = AverageMeter('Loss_seg', ':.4e')
    train_loader, train_loader_bg0, train_loader_bg1 = train_loader_list
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, loss_b, loss_s],
    ...
    for i, ((images, _), (bg0, _), (bg1, _)) in enumerate(zip(train_loader, train_loader_bg0, train_loader_bg1)):
        ...
        # compute output
        loss_byol, loss_dense = model(image_q, image_k, mask_q[:, 8::16, 8::16], mask_k[:, 8::16, 8::16])
        loss = loss_byol + loss_dense * .2

        loss_b.update(loss_byol.item(), images[0].size(0))
        ...
        if i % args.scalar_freq == 0 and torch.distributed.get_rank() == 0:
            ...
            writer.add_scalar('loss_moco', loss_byol.item(), global_step)
